...
class CalibrationDataset(MNN.data.Dataset):
    ...
    def __getitem__(self, index):
        image_name = os.path.join(
            self.image_folder, self.image_list[index].split(' ')[0])
        ...
        #!here is array do you know

        # after preprocessing the data, convert it to MNN data structure
        dv = F.const(image_data.flatten().tolist(), [
                     224, 224, 3], F.data_format.NHWC, F.dtype.float)
...
calibration_dataset = CalibrationDataset(
    image_folder='/data/imagenet_train_images')
