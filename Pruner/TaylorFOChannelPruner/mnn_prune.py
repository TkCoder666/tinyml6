...
if __name__ == '__main__':
    ...
    model.to(device)
    sparsity=0.5
    pruner = TaylorFOChannelPruner(model, total_pruning_iterations=5, sparsity=sparsity, debug_info=False, 
    prune_finetune_iterations=99,max_prune_ratio=0.99,
    align_channels=4 )

    prune_method="taylorfochannel"
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss, train_acc = train(
            model, get_train_loader(256, 16), optimizer, device, criterion)
        print(
            f"[ Train | {epoch :03d}/{epochs + 1:03d} ] loss = {train_loss:.5f}, acc = {train_acc:.5f}")
        valid_loss, valid_acc = test(
            model, get_test_loader(256, 16), device, criterion)
        print(
